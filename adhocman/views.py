...
from .forms import AdhocAttendanceIntimeForm,AdhocAttendanceUpdateOuttimeForm,AdhocPaymentForm,PGRNameForm
from dailyexpense.views import generate_unique_finance_requisition_number
from .forms import AdhocRequisitionStatusForm,AdhocRequisitionForm
from common.models import PGRdatabase
from.models import AdhocManRequisition,AdhocPayment,AdhocAttendance

from django.db.models import OuterRef, Subquery, Value, CharField,Sum, Value, DecimalField, DurationField, F, ExpressionWrapper,FloatField,Avg,Count,Q,Case, When, IntegerField,Max
...
def create_adhoc_requisition(request):
    ...
    adhocman_summary_form = AdhocRequisitionStatusForm()
    adhoc_requisitions = AdhocManRequisition.objects.all().order_by('-created_at')
    if request.method == 'POST':
        ...
        adhocman_summary_form = AdhocRequisitionStatusForm(request.GET)
        if adhocman_summary_form.is_valid():
            start_date = adhocman_summary_form.cleaned_data.get('start_date')
            end_date = adhocman_summary_form.cleaned_data.get('end_date')
            days = adhocman_summary_form.cleaned_data.get('days')
            region = adhocman_summary_form.cleaned_data.get('region')
            zone = adhocman_summary_form.cleaned_data.get('zone')
            mp = adhocman_summary_form.cleaned_data.get('mp')
    ...
    return render(request, 'adhocman/create_adhoc_requisition.html',{
        ...
        'adhocman_summary_form': adhocman_summary_form,
...
@login_required
def edit_adhocman_requisition(request, requisition_id):
    requisition_instance = get_object_or_404(AdhocManRequisition, id=requisition_id)  
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('adhocman:adhoc_approval_status2')
        else:
    ...
    return render(request, 'adhocman/adhoc_requisition_edit.html', {
        'form': form,       
...
def adhoc_approval_status2(request):
    ...
    form = AdhocRequisitionStatusForm(request.GET or {'days': 20})
    # adhoc_requisitions = AdhocManRequisition.objects.all().order_by('-created_at')
    adhoc_requisitions = AdhocManRequisition.objects.all().prefetch_related(
        'adhoc_man_requisition_ref'  # Related name from the ForeignKey in AdhocAttendance
    ).order_by('-created_at')
    ...
    page_obj = None 
    paginator = Paginator(adhoc_requisitions, 10)
    page_number = request.GET.get('page', 1)
    try:
        page_obj = paginator.page(page_number)
    except PageNotAnInteger:
...
def adhoc_management_approval(request, requisition_id):
    requisition = get_object_or_404(AdhocManRequisition, id=requisition_id)
    manager_level = request.user.manager_level
    if requisition.level1_approval_status == 'PENDING':
        required_level = 'first_level'
    elif requisition.level2_approval_status == 'PENDING' and requisition.level1_approval_status == 'APPROVED':
        required_level = 'second_level'
    elif requisition.level3_approval_status == 'PENDING' and requisition.level2_approval_status == 'APPROVED':
        required_level = 'third_level'
    else:
        return JsonResponse({"message": "Requisition already approved by all levels or lower levels not approved"}, status=400)
    if manager_level == required_level:
        ...
        messages.error(request, "You are not authorized to approve at this level or previous levels are not approved yet.")
        return redirect('adhocman:adhoc_approval_status')
...
def download_adhoc_requisition_data(request):
    ...
    requisitions = AdhocManRequisition.objects.all()
...
from.models import AdhocManRequisition
@login_required
def adhoc_intime2(request, requisition_id):
    ...
    if request.method == 'POST':
        ...
        form = AdhocAttendanceIntimeForm(initial={
            'adhoc_requisition': requisition_instance,
            'pgr': requisition_instance.pgr
        })


    return render(request, 'adhocman/adhoc_intime.html', {
        'form': form,
...
def fetch_requisitions(request, pgr_id):
    ...
    requisitions = AdhocManRequisition.objects.filter(
        pgr_id=pgr_id,      
...
def adhoc_outtime2(request, requisition_id):
    requisition_instance = get_object_or_404(AdhocManRequisition, id=requisition_id)
    adhoc_attendance = AdhocAttendance.objects.filter(adhoc_requisition=requisition_instance).first()

    if not adhoc_attendance:  # Create a new attendance object if it doesn't exist
        adhoc_attendance = AdhocAttendance(adhoc_requisition=requisition_instance, pgr=requisition_instance.pgr)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if adhoc_attendance.adhoc_requisition:
                adhoc_attendance.adhoc_requisition.active_status = False
                adhoc_attendance.adhoc_requisition.save()
    ...
    return render(request, 'adhocman/adhoc_intime2.html', {
        'form': form,
        'adhoc_attendance': adhoc_attendance,
        'requisition_instance': requisition_instance,
    })
...
def view_adhoc_attendance2(request):
    ...
    try:
        ...
        page_obj = paginator.page(paginator.num_pages)    
    for attendance in adhoc_attendance_data: 
        if attendance.adhoc_payment and attendance.adhoc_payment.adhoc_paid_amount is not None:
            attendance.remaining_amount = attendance.adhoc_bill_amount - attendance.adhoc_payment.adhoc_paid_amount
        else:
            attendance.remaining_amount = attendance.adhoc_bill_amount        
...
def create_adhoc_payment(request, adhoc_attendance_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('adhocman:view_adhoc_attendance2')
        else:
